...
async def test_user_reset_password_success():
    ...
    mock_repository.check_password = AsyncMock(
        return_value=(None, None)
    )
    mock_repository.change_password = AsyncMock(
        return_value=(None, None)
    )

    di[UserRepository] = mock_repository

    usecase = UserResetPasswordUsecase(salt)
    ...
    mock_repository.check_password.assert_called_once_with(
        user_id=user_id, 
        password=get_hashed_password(user_old_password, salt), 
    )
